...
namespace LP1_FOOP
{
    public class LogLineParser
    {
        ...
        {
            string pattern = @"^(?<processName>.+?)\s+\((?<processNumber>\d+)\),\s+(?<timestamp>\d{4}\.\d{2}\.\d{2}\s+\d{2}:\d{2}:\d{2}\.\d{3}),\s+(?<category>.+?),\s+(?<level>.+?),\s+(?<logMessageType>.+?),(?<messageContent>.*)$";
        ...
        private GenericStructuredLogMessage CreateGenericStructuredLogMessage(string line, Match match)
        {
            ...
            string level = match.Groups["levle"].Value.Trim();
            string lowMessageType = match.Groups["lowMessageType"].Value.Trim();
